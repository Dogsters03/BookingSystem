...
function buildValidationMessage(errors) {
  ...
  return `❌ Your request was blocked due to validation errors:\n\n${lines.join("\n")}\n\nPlease fix these fields and try again.`;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409) {
        const msg = `⚠️ Resource Already Exists\n\nA resource named "${payload.resourceName}" already exists in the system.\n\nPlease choose a different name or check the resources list to verify.`;
    ...
    msgLines.push("");
    msgLines.push(`Resource Name: ${body?.data?.name ?? ""}`);
    if (createdAt) msgLines.push(`Created at: ${createdAt}`);
    msgLines.push(`Resource ID: ${body?.data?.id ?? ""}`);
    msgLines.push("");
    msgLines.push(`The resource is now available in your booking system.`);
    ...
    showFormMessage("error", "❌ Network Error\n\nCould not reach the server. Please check your connection and try again.");
  }
}
