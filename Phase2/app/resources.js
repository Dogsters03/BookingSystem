...
// Will hold references to form elements and buttons
...
function isResourceDescriptionValid(value) {
  ...
  // Allowed: letters, numbers, Finnish letters, spaces, comma, period
  const allowedPattern = /^[a-zA-Z0-9äöåÄÖÅ ,.]+$/;

  const lengthValid = trimmed.length >= 10 && trimmed.length <= 500;
  const charactersValid = allowedPattern.test(trimmed);
...
function updateCreateButtonState() {
  // Create button is enabled only if ALL validations pass
  const allValid = validationState.resourceName && validationState.resourceDescription;
...
// Attach validation to description textarea
